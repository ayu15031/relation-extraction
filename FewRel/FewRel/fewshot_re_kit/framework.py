...
class FewShotREModel(nn.Module):
    def __init__(self, my_sentence_encoder):
        ...
        if my_sentence_encoder is not None:
...
class FewShotREFramework:
    ...
    def train(self,
        ...
        for it in range(start_iter, start_iter + train_iter):
            ...
            accuracies_with_iter.append([it + 1, 100 * iter_right / iter_sample])
        ...
        f = open(f"train_output/{model_name}_N_K.json", "w")

        json.dump(accuracies_with_iter, f)
                
        print("\n####################\n")
        print("Finish training " + model_name)


    def cal_roc(self, label, pred, N):
        label = np.array(label)
        pred = np.array(pred)

        cnf_matrix = sklearn.metrics.confusion_matrix(label, pred)
        ...
        # auc = sklearn.metrics.roc_auc_score(label, pred, average="micro", multi_class="ovo")
        fptp = [(FPR[i], TPR[i]) for i in range(len(FPR))]
        fptp = sorted(fptp, key=lambda x: x[0])
        print(fptp, "hufhuhggi")
        auc = sklearn.metrics.auc(fptp[0], fptp[1])

        metrics = {"TPR": list(TPR), "FPR": list(FPR), "auc":auc}
    ...
    def eval(self,
        ...
        with torch.no_grad():
            for it in range(eval_iter):
                ...
                if model_name:
                    print("F1 score is : ---- -----", sklearn.metrics.f1_score(label.cpu(), pred.cpu(), average="micro"))

                    metrics = self.cal_roc(label.cpu(), pred.cpu(), N)
                    print(metrics)

                    with open(f"train_output/{model_name}_{N}_{K}.json", "w") as outfile:
