...
def main():
    ...
    if model_name == 'proto':
        ...
        model = MetaNet(N, K, sentence_encoder, max_length)
    ...
    acc = framework.eval(model, batch_size, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair, model_name=model_name)
    print("RESULT: %.2f" % (acc * 100))

    
    #save model output
    # output_path = './pretrained_models'
    # output_name = model_name + '.pt'
    # torch.save(model.state_dict(), os.path.join(output_path, output_name))
